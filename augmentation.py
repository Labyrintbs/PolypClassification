...
DATASET_ROOT = "/users/Etu0/21400500/Polar_Dataset"
OUTPUT_DIR = "/users/Etu0/21400500/Polar_Dataset/augmented_hyperplastic_images"
...
for _, row in tqdm(df_hyper.iterrows(), total=len(df_hyper)):
    ...
    for i in range(AUG_PER_IMAGE):
        try:
            ...
            aug_records.append({
                "IMG_FILE_PREFIX": aug_filename.replace(".png", ""),
                "PATIENT_ID": row['PATIENT_ID'],
                "POLYP_ID": row['POLYP_ID'],
                "POLYP POSITION (x,y,w,h)": "",  # Not used now
...
df_combined.to_csv("polar_dataset_combined.csv", index=False)

print(f"✅ Combined CSV saved to: polar_dataset_combined.csv ({len(df_combined)} total entries)")
